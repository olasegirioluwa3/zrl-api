...
async function registerUser(req, res, data) {
    try {
      ...
      const verifyLink = `${domain}/verify-email/${token}`;
...
async function loginUser(req, res, data) {
  try {
    ...
    if (!user) {
      ...
      return res.status(401).json({ message: 'Account not activated! Please, check your email for verification link' });
    }
    const passwordMatch = await bcrypt.compare(password, user.password);
    if (!passwordMatch) {
      return res.status(401).json({ message: 'Invalid Password' });
    }
    const token = jwt.sign({ userId: user.id, userRole: user.role }, process.env.APP_JWT_SECRET_KEY, { expiresIn: '24h' });
...
async function updateCoverPicture(req, res) {
  ...
  async function getUserByUsername(req, res) {
    try {
      ...
      const user = await User.findOne({ where: { username }, exclude: {password}});
  ...
  async function verifyAccountEmail(req, res, data) {
    try {
      ...
      if (await sendEmail(user.email, 'Account verified successfully', emailText)){
        await user.save();
        res.status(200).json({ status: "success", message: 'Account email verified successfully, proceed to login', user });
      }
    } catch (error) {
      console.error(error);
  ...
  async function resetPasswordInit(req, res, data) {
    try {
      ...
      res.status(200).json({ status: "success", message: 'Reset Email Token is Valid', email: user.email });
    } catch (error) {
      console.error(error);
      res.status(500).json({ message: 'Failed to reset password' });
    }
  ...
  async function resetPasswordFinal(req, res, data) {
    try {
      ...
      if (await sendEmail(user.email, 'Reset Password Successfully', emailText)){
        await user.save();
        res.status(200).json({ status: "success", message: 'Password reset was successful', email: user.email });
      }
    } catch (error) {
      console.error(error);
      res.status(500).json({ message: 'Failed to reset password' });
